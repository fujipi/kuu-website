...
3. 既存の `content/blog/*.mdx` ファイル名を確認し、slugが重複しないことを検証。さらに `node scripts/validate-blog.mjs` の `near_duplicate` 警告で、新規の title / tag / `target_query` が既存記事とテーマ重複しないことを確認する（重複する場合は新規生成を中止し、既存記事の更新に切り替える。正当に読者層を分けて併存させる場合のみ `scripts/validate-blog.mjs` の `NEAR_DUP_ALLOWLIST` にペアを登録）
...
> 近接重複により統合済みの旧記事は `/blog/<canonical>/` へリダイレクトする（マップは `src/lib/blog-redirects.ts`）。コスト/ROI 系は `/blog/ai-investment-cost-guide/` に集約済みで、新規のコスト・費用・ROI 主題は Blog では作らず Case（`/case/`）で扱う。
...
- `kuu-editorial` — 上記以外（戦略・経営寄りの記事を含む。未指定時のデフォルト）
...
```mdx
---
title: "記事タイトル（全角32字以内・主要KWを前方・ブランド名は入れない）"
description: "70〜120字。1文目で target_query に直接回答し、数値/固有名詞を1つ以上"
date: "YYYY-MM-DD"
lastModified: "YYYY-MM-DD"
tags: ["タグ1", "タグ2", "タグ3"]
author: "kuu-engineering"           # kuu-engineering / kuu-editorial（content/authors/*.json と対応、未指定時はkuu-editorial）
...
> `title` と `description` はGoogle検索結果の「リンク」と「説明文」にそのまま出る。専門性・権威性が伝わり、かつ truncation で切れないコピーにする。

- **`title`（frontmatter）**: 全角**32字以内**を目安（Googleは約30〜35字で切る）。主要キーワード／記事の答えを**前方**に置く。**ブランド名「Kuu株式会社」は title に含めない**（記事ページ側でブランドは付与せず、検索エンジンが自動補完する）。`audience: "smb"` 以外では「中小企業」「経営者向け」を冠しない。
- **`description`（frontmatter）**: **70〜120字**。1文目で `target_query` に直接回答し、数値または固有名詞を1つ以上含める。`audience` に文体を合わせる（enterprise はスケール・統制、smb は着手手順）。
...
- 主要なH2は可能なら**質問形**（「〜とは何か」「〜すべきか」「〜ですか」等）にして直後にDABを置く。質問形H2＋回答が2ペア以上ある記事はビルド時に FAQPage 構造化データが自動付与される（AEO施策。`src/lib/faq.ts`）
...
- `title` / `description` に定型フィラーを使わない: 「実践的な情報を発信しています」「徹底解説」「わかりやすく解説」「〜のすべて」等
...
- [ ] **重複チェック（強化）**: 既存 `content/blog/*.mdx` の slug・title・`target_query`・tag セットと近接重複しない。`pnpm validate:blog` の `near_duplicate` 警告が新規 slug に出ていないことを確認する（出た場合は (a) 既存記事への追記に切替、(b) 切り口・`audience` を明確に変える、(c) 正当に併存するなら `scripts/validate-blog.mjs` の `NEAR_DUP_ALLOWLIST` に登録、のいずれか）。`node scripts/blog-coverage-report.mjs` で同 `track`×`audience` の既存本数も確認した
