...
リモートに残留していた作業ブランチ **116 本**の棚卸し結果と、削除対象一覧。
基準時点の `main` は `283ac3d89ad8e318282ab3baf92bc27a0f636c6f`。

> **状態: 削除は未実行。** 削除を生成したセッションの git プロキシが ref の削除だけを
> HTTP 403 で拒否したため（ref の作成・更新は通る）、実行は手元に委ねる。
> 手順は下記「削除の実行」を参照。
...
判定には全履歴が要る。セッションの clone は shallow だったため `git fetch --unshallow` してから判定している（shallow のままだと古いブランチが軒並み「未マージ」と誤判定される）。
...
C 群 15 本は PR #97〜#121 の滞留事故（CLAUDE.md 記載）で生成され、**PR #122 がテーマ重複と判定して破棄した記事**にあたる。15 本すべてについて `main` 側に同テーマの記事が存在することを確認したうえで削除対象に含めた（例: `computer-use-*` 系は `main` に3本、`shuro-keizoku-shien-b-agent` は `disability-work-support-record-agent` が対応）。

なお本ブランチ `claude/cleanup-unused-branches-y532ew` と、棚卸し後に出現した `claude/error-investigation-fp5t6s` は対象外。

## 削除の実行

```
./scripts/delete-stale-branches.sh --dry-run   # 対象と差分の確認のみ
./scripts/delete-stale-branches.sh             # 実削除
```

スクリプトは本ファイルの表から「ブランチ名と SHA」を読み取り、**リモートの現在の
SHA が記録と一致する場合のみ**削除する。棚卸し後に誰かが push し直したブランチは
スキップして報告するので、取りこぼしを消す事故が起きない。

## 復元方法

削除したブランチは SHA が分かれば復元できる。下表の SHA を使う。
...
削除するとこの記事本文はリポジトリから消える。必要なら削除前に取り出すこと。
